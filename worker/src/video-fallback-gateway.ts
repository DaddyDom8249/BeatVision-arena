...
export default { async fetch(r: Request, e: any) {
  ...
  if (path === '/' || path === '/health') {
    return json(r, e, { ok: true, status: 'online', name: 'BeatVision Provider Gateway', contract_version: '1.1', primary_motion_provider: 'pixazo', assembly_provider: 'shotstack-sandbox', fallback_provider: 'shotstack-camera-motion', request_id: id });
  }
  ...
  if (path === '/v1/video/animate') {
    ...
    // Do NOT call Hugging Face here. The previous fallback could consume the
    // user's included inference quota and turn a free test into a billing wall.
    // Shotstack Sandbox provides a real MP4 camera-motion fallback without an
    // AI-video generation charge: the approved image gets deterministic motion.
